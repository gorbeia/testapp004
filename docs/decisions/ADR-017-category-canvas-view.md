...
- **Entry point:** A hub-icon button on each occupied category row in the Categories screen.
...
- **Pan/zoom:** `detectTransformGestures` with centroid-based zoom so pinch always zooms around the pinch point.
...
- **Edge labels:** Uses `Relation.labelFor(fromId)` to derive a perspective-aware directional label.
